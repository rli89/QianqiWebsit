...
function renderTopbar(user){
    ...
    exit.onclick = function() {
        ...
        location.href = "/code/admin/views/login/index.html"
    }
...
async function load(id) {
    ...
    if (user){
        ...
        let topbarText = await fetch("/code/admin/components/topbar/index.html").then(res=>res.text())
       
        document.querySelector(".topbar").innerHTML = topbarText
        renderTopbar(JSON.parse(user))

         
        let sidemenuText = await fetch("/code/admin/components/sidemenu/index.html").then(res=>res.text())
